...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.pixel.device | grep -q "$LINEAGE_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.pixel.device | grep -q "$LINEAGE_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.pixel.device | grep -q "$LINEAGE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
